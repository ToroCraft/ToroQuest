...
public class LayerCape implements LayerRenderer<EntityLivingBase> {
	private final RenderGuard playerRenderer;

	public LayerCape(RenderGuard playerRendererIn) {
	...
	public void doRenderLayer(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
		...
		this.playerRenderer.bindTexture(TEXTURES);
		...
		f1 = f1 + MathHelper.sin((entitylivingbaseIn.prevDistanceWalkedModified + (entitylivingbaseIn.distanceWalkedModified - entitylivingbaseIn.prevDistanceWalkedModified) * partialTicks) * 6.0F) * 32.0F * 1;
